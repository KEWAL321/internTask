...
$stmt = $conn->prepare("SELECT s.id,u.name,u.email,s.address,s.phone,c.class_name FROM students AS s 
...
<body>
    ...
    <div class='flex flex-row rounded-[20px] h-[100%] w-[100%] justify-center items-center'>
        ...
        <div
            class='h-[90%] w-[50%] py-10 rounded-r-[16px] bg-gray-400/50 grid grid-flow-col grid-rows-3 text-center border border-black-500'>
                        ...
                        <?php 
                                foreach ($allStudents as $student) {
                                    ...
                                    <tr>
                                        <td>{$student['id']}</td>
                                        <td>{$student['name']}</td>
                                        <td style='color:gray;'>".($student['class_name'] ?? 'Not Set')."</td>
